        ...
        if is_shaped:
            ...
            for shape_index, deforms in enumerate(shape_deforms):
                ...
                try:
                    ...
                    self.report(type={'WARNING'}, message=f_("Could not apply '%s' modifier \"%s\" to shapekey %i", mod.type, mod.name, shape_index))
        ...
        for index, mod in enumerate(copy_modifiers):
            ...
            if self.is_applies[index].value and (mod.type != 'ARMATURE' or not compat.IS_LEGACY):
                ...
                try:
                    ...
                    #ob.modifiers.remove(mod)
                    self.report(type={'ERROR', 'WARNING'}, message=f_("Could not apply '{type}' modifier \"{name}\"", type=mod.type, name=mod.name))
                    print(f_("Error applying '{type}' modifier \"{name}\":\n\t", type=mod.type, name=mod.name), e)
        ...
        for index, mod in enumerate(copy_modifiers):
            ...
            if self.is_applies[index].value and (mod.type == 'ARMATURE' and compat.IS_LEGACY):
                try:
                    ...
                    #ob.modifiers.remove(mod)
                    self.report(type={'ERROR', 'WARNING'}, message=f_("Could not apply '{mod_type}' modifier \"{mod_name}\"", mod_type=mod.type, mod_name=mod.name) )
