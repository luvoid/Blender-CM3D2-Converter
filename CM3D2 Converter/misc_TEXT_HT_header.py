...
def menu_func(self, context):
    ...
    if 'BoneData' in text_keys:
        ...
        for line in txt.as_string().split('\n'):
            if line:
                line_count += 1
        row.operator('text.show_text', icon='ARMATURE_DATA', text=f_iface("BoneData ({})", line_count, msgctxt='Operator')).name = 'BoneData'
    if 'LocalBoneData' in text_keys:
        ...
        for line in txt.as_string().split('\n'):
            if line:
                line_count += 1
        row.operator('text.show_text', icon='BONE_DATA', text=f_iface("LocalBoneData ({})", line_count, msgctxt='Operator')).name = 'LocalBoneData'
