...
class CNV_OT_vertices_count_checker(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if inner_count <= 65535:
            self.report(type={'INFO'}, message=f_("○ 出力可能な頂点数です、あと約%d頂点ほど余裕があります (頂点数:%d(+%d) UV分割で増加:+%d％)", 65535 - inner_count, real_count, inner_count - real_count, int(inner_count / real_count * 100)))
        else:
            self.report(type={'ERROR'}, message=f_("× 出力できない頂点数です、あと約%d頂点減らしてください (頂点数:%d(+%d) UV分割で増加:+%d％)", inner_count - 65535, real_count, inner_count - real_count, int(inner_count / real_count * 100)))
